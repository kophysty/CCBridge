...
AC-21   Event-driven UX: все renderer'ы получают одинаковый поток
        CCBridgeEvent. RichRenderer работает в Stop hook.
        JsonlRenderer пишет в audit.jsonl. ccbridge audit watch
        запускается во втором терминале и live-обновляется
...
│
├── renderers/                # UI-слой, отделён от логики (Wave-ready)
│   ├── __init__.py
│   ├── base.py               # Renderer Protocol
│   ├── rich_renderer.py      # Красивый rich UI для Stop hook stdout
│   ├── jsonl_renderer.py     # Пишет события в audit.jsonl как stream
...
AC-21   Event-driven UX (Wave-ready): RichRenderer работает в Stop hook
        stdout. JsonlRenderer пишет события в audit.jsonl как stream.
        ccbridge audit watch во втором терминале live-обновляется при
        запуске audit run в первом (события появляются < 1 сек).
        Все renderer'ы получают одинаковый поток CCBridgeEvent через
        EventBus — добавление WaveRenderer (v0.2) и MCPRenderer (v0.3)
        не требует изменений в orchestrator/core.
```
...
- **2026-05-02 v0.0.3** — Добавлен §2.9 «UX & Event-driven rendering»
  (Wave-readiness): EventBus + три renderer'а (rich/jsonl/silent) в v0.1,
